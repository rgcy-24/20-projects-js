...
function checkGuess() {
  ...
  let checkGuessLength = new Set(guess);
...
function printRules() {
  alert("Entrez un nombre composé de 4 chiffres différents dans la case à côté de 'Guess'. L'ordinateur le compare avec le code secret et vous donne deux indices: les numéros des 'bulls' (B) et des 'cows' (C). Qu'est ce que cela signifie? Un 'bulls' est un chiffre qui est présent dans les deux codes à la même position. Un 'cows' est un chiffre qui est présent dans les deux codes mais à une position différente. Par exemple, si le code secret est 7512 et que vous essayez 5392, la réponse sera '1B 1C' (1 bull 1 cow). C'est tout!");
}